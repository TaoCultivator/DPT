...
- 修复 `NDE36_MCU24A1` 等慢关断波形的 `Eoff` 默认光标：关断前 Vce 低值平台的搜索范围不再被错误截到主边沿内部，`Ha` 继续表示稳定低压平台，而不是边沿中间值或过冲最大值。点名的 `RT/UH2_915V_400A_000.tss` 自动结果为 `Ha=19.7481 V、A=4.545344 us、B=4.913379 us、Eoff=40.0242 mJ`；按用户截图手调到 `Ha=18.187 V、A=4.509 us、B=4.913 us` 时复现 `Pmax=346.405 kW、Eoff=40.391 mJ`。
- 修复参数光标已手调、切换参数后再次点击时，A/B 仍有效但处于当前局部视图之外，导致看不到竖光标和标签的问题；重新点击 `dv/dt`、`Eoff/Eon/Err` 或功率项时，只有在光标离屏的情况下才重新聚焦，正常横向拖动不会突然跳页。
- 双模块报告按文件名中的模块编号保存独立测试项，不再用未编号的 `UH/UL` 块互相覆盖；固定排序为 `UH1 → UL1 → UH2 → UL2`，先写模块 2、后写模块 1 时也会把模块 1 插入正确位置，并兼容已有未编号的模块 1 报告块。
- 再次确认报告进度在同一任务内只允许单调增加，保存成功前不显示 100%；合并单元格写入统一定位左上锚点，不再写入只读 `MergedCell`。
- `NDE36_MCU24A1` 全部 8 个原始 TSS 严格重算通过；当前全部 450 个原始 TSS 无缓存严格重算为双脉冲 361、单脉冲 16、短路 73，`failed=0、warnings=0`。全量关键 GUI 审计覆盖 377 个非短路文件、1508 项，`OK=1476、INFO=32、FAIL=0`，INFO 均为单脉冲不适用项。
- 清洁源码分组完整测试合计 `717 passed、89 skipped、378 subtests passed、0 failed`。本版不包含已暂缓的 TekVISA 实机连接调整；固定非商业用途授权海报未修改。

## v3.0.8 更新

- 关断、开通和反向恢复 `dv/dt` 统一使用事件内稳定高/低电压平台作为 `Top/Base`，按 `Base + pct × (Top - Base)` 求原始波形交点；过冲、振铃和全窗最大值不再冒充水平参考线。`Vrr`、`Ic_off_max` 等名称明确包含峰值/最大值的参数继续独立取峰值。
- 修复报告写入进度可能因同一任务重复开始或迟到回调而倒退的问题：运行中百分比只允许单调增加，成功完成时才到 100%。
- 修复用户报告模板把数据槽合并后写入时报 `'MergedCell' object attribute 'value' is read-only` 的问题；写入会自动定位合并区域左上锚点，同时保留普通单元格行为。
- 用户指定的 `07CF2C1000 20260717` 项目重新验证全部 74 个原始 TSS：双脉冲 56、短路 18，失败和警告均为 0；三类 dv/dt GUI 光标共 168 项全部通过。
- 当前全部 450 个原始 TSS 无缓存严格重算为双脉冲 361、单脉冲 16、短路 73，`failed=0、warnings=0`；所有适用文件的三类 dv/dt GUI 光标审计共 1131 项，`OK=1099、INFO=32、FAIL=0`，INFO 均为单脉冲不适用项。
- 本版不包含尚未完成硬件终验的 TekVISA 连接调整；固定非商业用途授权海报未修改。

## v3.0.7 更新

- Tektronix USB 直读改为优先使用 TekVISA，未安装、无法创建资源管理器或身份发现失败时再回退到 NI-VISA；两套同时安装时固定选择 TekVISA。波形传输或 Zoom/光标同步一旦开始，超时、二进制块、点数和长度等数据完整性错误会直接失败，不再静默换后端重试。
- Windows 单文件包新增内置 TekVISA 原生适配源码；VISA 运行时仍由目标电脑安装，推荐 TekVISA V5.11.1，也兼容 NI-VISA 26.0.x。实机 MSO46B 已通过 TekVISA 完成 7 个源、每源 125000 点的当前记录采集。
- 切换顶部相别或上下桥后，重新计算会停留在全局波形视图，不再重新进入上一个参数的局部放大页；应用启动时会直接恢复上次相别、桥臂、温度和对应报告工况。
- 测试夹具改为显式临时 INI 设置存储，完整测试不再访问或清理用户真实的 Windows 注册表设置。
- 本次未修改双脉冲或短路参数数值、Top/Base、积分、斜率、时间窗和默认光标落点。发布前完整测试为 `707 passed、88 skipped、393 subtests passed、0 failed`；固定非商业用途授权海报未修改，SHA-256 保持 `0042390506059EE1DEEBE810B0BAE52C5F0C7B4BBB672910757659B10630A784`。

## v3.0.6 更新

- 顶部测试条件现在会跨文件和应用重启记住上次设置：相别、桥臂、温度工况、RT/HT/LT 实际温度以及双脉冲/短路报告工况输入均会保留；导入数据时只有路径或示波器元数据明确识别到的字段才覆盖，未识别字段继续沿用上次值。
- 记忆参数按桥臂分成两套：UH/VH/WH 共用上桥参数，UL/VL/WL 共用下桥参数；不同相别不再产生六套重复配置，并兼容迁移旧的按相别保存的报告工况。
- 短路应力 `Vpeak` 在 DUT Vge 无法形成有效 Base A/B 事件窗时，改用短路电流的有效 A/B/Hb 作为共同事件窗，分别提取本管与对管 Vce 在窗内的原始最大值；事件窗外异常尖峰不会计入。Vge 事件窗有效时仍保持原有取值口径。
- 本次未修改 `Imax`、`Tsc`、`Esc`、Desat 或双脉冲 DPT 的计算口径。发布前完整测试为 `705 passed、88 skipped、393 subtests passed、0 failed`；重新扫描全部 `428` 个原始 TSS，得到双脉冲 `353`、单脉冲 `16`、短路 `59`，`failed=0、warnings=0`；全部 `59` 个短路样例 GUI 光标审计共 `413` 项，`FAIL=0`。固定非商业用途授权海报未修改，SHA-256 保持 `0042390506059EE1DEEBE810B0BAE52C5F0C7B4BBB672910757659B10630A784`。

## v3.0.5 更新

- 换流回路杂散电感 `Ls_on/Ls_off` 改为电压面积与电流变化严格同窗的 `L=∫uLdt/|ΔI|`：开通使用 Ic 上升 A/B，同窗积分 `max(Vce Top-Vce,0)`；关断在 Ic 下降边界内提取主 Vce 正过冲瓣并与同窗 `ΔIc` 配对，避免旧的单点 `ΔVce/平均 di/dt` 在不同工况间产生系统偏差。
- `Delta Vce` 仍是 Ls 的电压幅值校准量：手动调整 `Delta Vce` 后，当前 Ls 按 `Delta Vce manual/automatic` 比例实时更新；Ls 独立 A/B 只调整同步积分窗，不改写 `Delta Vce` 或 `di/dt`。
- 短路时间 `Tsc` 取值范围新增 `0.0%~99.9%` 的自定义对称阈值，A/B 继续取原始短路电流上升沿和下降沿与同一阈值的真实交点；原有 `0%-0%`、`10%-10%` 保持兼容。
- 每次 USB 示波器新采集都会重新采用当前记录的通道 Scale、Position、Unit 与反相元数据；同一 `scope://<serial>` 下的新波形不再继承上一条记录的显示设置。报告工况输入框会在固定工具栏宽度内按内容自适应分配宽度。
- 指定校准集 `285/805/1050A` 的三电流最大跨度为 `Ls_on 9.63%`、`Ls_off 8.35%`；独立数据 `403/806/1048A` 为 `Ls_on 14.37%`、`Ls_off 5.87%`，低电流差异未被跨样本归一化或固定系数压平。
- 发布前完整测试为 `696 passed、86 skipped、389 subtests passed、0 failed`；重新扫描当前全部 `428` 个原始 TSS，得到双脉冲 `353`、单脉冲 `16`、短路 `59`，`failed=0、warnings=0`；代表性 GUI 光标审计 `416` 项、`FAIL=0`。固定非商业用途授权海报未修改，SHA-256 保持 `0042390506059EE1DEEBE810B0BAE52C5F0C7B4BBB672910757659B10630A784`。

## v3.0.4 更新

- 六个关断、开通和反向恢复 `dv/dt`/`di/dt` 的“自动最大斜率”统一改为固定 `20%` 幅值跨度；程序仍在既有主沿内寻找最大且连续、单调、近似直线的有效区间，最终数值与 A/B 光标继续由原始波形真实交点计算。
- 参数表取值范围列只显示实际百分比（如 `46%→66%`），不再附加时间文本或撑宽列；自动模式用左下角红色“自”和右下角红色“动”独立标识，百分比保持单次绘制、完整垂直居中，角标会避开主文字。
- 六个斜率项现在会跨 TSS 文件和应用重启记住上次选择的预设或自定义范围；自动最大斜率只记住“自动”模式，不保存某个波形算出的百分比，因此加载新波形后仍会重新搜索实际 20% 区间。
- 本次未修改除用户选择“自动最大斜率”后对应斜率范围之外的 Top/Base、积分、损耗窗口、默认预设、其他参数数值或报告字段。
- 发布前完整测试为 `692 passed、86 skipped、389 subtests passed、0 failed`；默认配置与六项自动斜率配置分别重新扫描全部 `388` 个原始 TSS，均为 `failed=0、warnings=0`。固定非商业用途授权海报未修改，SHA-256 保持 `0042390506059EE1DEEBE810B0BAE52C5F0C7B4BBB672910757659B10630A784`。

## v3.0.3 更新

- 修复参数局部放大页面手调后，点击水平标度栏右侧 `X` 返回全局，再次点击同一参数仍停留在全局、无法回到刚才手调页面的问题。
- `X`/全图预览切到全局时，现在会保存退出瞬间的活动参数和精确局部 X 轴窗口；再次点击同一参数时，先恢复已有 A/B、Ha/Hb 与交互上下文，再恢复该局部窗口。
- 待恢复窗口只属于退出时的参数；点击其他参数或加载新波形会清除该状态，避免把上一参数或上一文件的窗口错误套用。通过预览按钮直接切回局部仍恢复同一窗口。
- 恢复后的软件窗口与手调光标继续只触发一次 Tektronix Zoom/光标同步；`X` 退出仍同步关闭示波器 Zoom，不需要在示波器上重复操作。
- 本次只修复视图状态所有权和重进流程，没有修改参数数值、Top/Base、默认 A/B/Ha/Hb 卡位、斜率、积分、时间窗或报告字段。
- 发布前完整测试为 `688 passed、86 skipped、389 subtests passed、0 failed`；默认配置重新扫描全部 `388` 个原始 TSS，结果为 `failed=0、warnings=0`。固定非商业用途授权海报未修改，SHA-256 保持 `0042390506059EE1DEEBE810B0BAE52C5F0C7B4BBB672910757659B10630A784`。
...
通过 USB 直读 Tektronix 示波器时，电脑至少安装一套 VISA 运行时：推荐安装 **TekVISA V5.11.1（066093813）**，并将其作为第一优先级；已有 **NI-VISA 26.0.x** 的电脑继续作为兼容回退。两套同时安装时软件优先使用 TekVISA。仅打开 TSS 文件不需要安装 VISA。示波器桥接使用 Windows 10/11 自带的 Windows PowerShell 5.1，不要求额外安装 PowerShell 7。
