...
- 新增 Pdmax 功率参数、报告模板列和功率波形局部查看，反向恢复 Pdmax 按绝对功率显示。
...
导出文件包含单 Sheet「`*_双脉冲数据`」，表头 4 行（信息 / 关断 / 开通+反向恢复 / 列名+单位），**第 5 行**写入本次提取结果。列序 A–AN 与 MCU2506 规范一致（含关断/开通/反向恢复 Pdmax、Eoff/Eon/Err 等）。
