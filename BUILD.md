...
- Tektronix USB 直读运行时不打入 exe：目标电脑推荐安装 TekVISA V5.11.1（第一优先级），也兼容 NI-VISA 26.0.x 回退；只使用 TSS 时无需安装 VISA
...
```powershell
pyinstaller --noconfirm --windowed --name DPT_双脉冲参数提取工具_v{版本号} --collect-all PyQt6 --collect-all pyqtgraph main.py
```
