...
class TestExcelExport(unittest.TestCase):
    ...
    def test_generated_workbook_layout_and_data_row(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(ws.cell(HEADER_NAME_ROW, COL_OFF["pmax"]).value, "Pmax")
            self.assertEqual(ws.cell(HEADER_NAME_ROW, COL_ON["pmax"]).value, "Pmax")
            self.assertEqual(ws.cell(HEADER_NAME_ROW, COL_RR["pdmax"]).value, "Pdmax")
            self.assertGreater(float(ws.cell(DATA_ROW, COL_OFF["pmax"]).value), 0)
            self.assertGreater(float(ws.cell(DATA_ROW, COL_OFF["eoff"]).value), 0)
            self.assertGreater(float(ws.cell(DATA_ROW, COL_ON["pmax"]).value), 0)
